...
import type { OurStoryContent } from '../../content/aboutContent';
...
export const OurStory: React.FC<OurStoryProps> = ({ content }) => {
  const storyPhases = content.phases;
  ...
  useEffect(() => {
    setActiveImage(storyPhases[0]?.image || '');
  }, [storyPhases]);  
  ...
  useGSAP(() => {
    ...
    mm.add("(max-width: 1023px)", () => {
      ...
      setTimelinePosition();
      setTimeout(setTimelinePosition, 150);
  ...
  scope: containerRef,
  dependencies: [storyPhases.length, content.heading],
  revertOnUpdate: true,
});
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[0.3fr_0.1fr_0.6fr] gap-12 lg:gap-0 max-w-7xl mx-auto">
        ...
        <SectionHeader label={content.sectionLabel} className="text-black" />
          <CMSHeading
            ...
            className="text-[32px] md:text-[48px] font-medium leading-[1.1] tracking-tight text-black font-agrandir"
          />
        </div>
        ...
        <div className="relative flex flex-col gap-32 lg:gap-64 pl-10 lg:pl-0 mt-20 lg:mt-0">
          ...
          <div className="hidden lg:block absolute right-0 top-0 h-full w-[50%] pointer-events-none">
            ...
            >
              ...
              onError={(e) => {
                const fallback = storyPhases.find((phase) => phase.image === activeImage)?.fallbackImage;
              ...
              <div className="content flex flex-col gap-6">
                ...
                <div className="lg:hidden aspect-[16/10] w-full overflow-hidden rounded-sm bg-zinc-100 mt-4 md:max-w-[400px]">
                  <img 
                    ...
                    onLoad={() => ScrollTrigger.refresh()}
                    onError={(e) => {
                      if (phase.fallbackImage) e.currentTarget.src = phase.fallbackImage;
                    }}
                  />
                </div>
              </div>
