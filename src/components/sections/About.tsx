...
/**
 * Frontend is the source of truth for Home About icons.
 * Do not use CMS/API icon values here.
 */
const ABOUT_FEATURE_ICONS = [Plug, Bridge, Handshake];
...
export const About: React.FC<AboutProps> = ({ content, canAnimate = true }) => {
  ...
  useGSAP(() => {
    ...
    scope: containerRef,
    dependencies: [fullText, canAnimate],
    revertOnUpdate: true,
  });
  ...
  return (
    <section
      ref={containerRef}
      className="relative min-h-screen w-full bg-white flex flex-col justify-start lg:justify-between gap-16 lg:gap-24 pt-24 pb-32 px-8 md:px-16 lg:overflow-hidden"
    >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[0.3fr_0.7fr] gap-12 lg:gap-24 mt-12 lg:mt-24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-16">
            {content.features.map((feature, i) => {
              const FeatureIcon =
                ABOUT_FEATURE_ICONS[i] || ABOUT_FEATURE_ICONS[0];

              return (
                <div
                  key={`${feature.title}-${i}`}
                  className="feature-card flex flex-col gap-8 border-l border-zinc-200 pl-8"
                >
                  <div className="w-16 h-16 bg-zinc-50 flex items-center justify-center rounded-sm">
                    <FeatureIcon size={32} weight="light" className="text-black" />
                  </div>
